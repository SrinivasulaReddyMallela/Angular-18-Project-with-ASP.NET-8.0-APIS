...
namespace WebGYM.API.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllPayment([FromQuery] QueryParameters queryParameters)
        {
            try
            {
                var userId = Convert.ToInt32(this.User.FindFirstValue(ClaimTypes.Name));
                List<PaymentDetailsViewModel> allMembers =(await _paymentDetails.GetAll(queryParameters, userId)).ToList();
                var allItemCount = await _paymentDetails.Count(userId);
