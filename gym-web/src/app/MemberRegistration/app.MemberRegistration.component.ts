...
export class MemberRegistrationComponent implements OnInit {
    ...
    MemberModel: MemberRegistrationModel |any={};
    errorMessage: any;
    output: any;
    genderList: any[]=[];
    bsRangeValue: Date[]=[];
    bsValue = new Date();
    joinminDate: Date=new Date();
    joinmaxDate: Date=new Date();
   
    dobminDate: Date=new Date();
    ...
    onSubmit() {
        ...
        this.MemberModel.PlanName="test";
        this.MemberModel.SchemeName="test";
    ...
    CalcuateAge(birthdate:any) {
        if (birthdate) {
            var timeDiff = Math.abs(Date.now() - birthdate);
            this.MemberModel.Age = Math.floor((timeDiff / (1000 * 3600 * 24)) / 365).toString();
        }
    }
    ...
    GetAmount(PlanID: number, SchemeID: number) {
        if (PlanID != null && SchemeID != null) {
            this._memberregistration.GetAmount(PlanID.toString(), SchemeID.toString()).subscribe(
                amount => {
